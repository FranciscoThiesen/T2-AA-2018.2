...
{
    int numeroDeNos;
    deque<bool> mascaraTerminal; // Usando deque, porque vector tem um overhead maior com bool do que deque
    
    vector< aresta > arestas;
    
    grafo(int N) : numeroDeNos(N) { mascaraTerminal.assign( N, false ); } 
    
    void definirNosTerminais( std::vector<int> indicesDosNosTerminais )
    {
        mascaraTerminal.fill( mascaraTerminal.begin(), mascaraTerminal.end(), false);        
        for(const int& indice : indicesDosNosTerminais) mascaraTerminal[indice] = true;
    }
    
    void adicionaAresta( int orig, int dest, int c) { arestas.push_back( aresta( orig, dest, c) ); }
